...
SELECT PRO_NO, PRO_NAME, PRO_PRICE, PRO_DATE, PRO_MODIFY_DATE, PRO_VOL, PRO_IMG, PRO_INFO, PRO_STAT, PRO_CAT, PRO_SOLD, PRO_MFG, PRO_RATING
FROM PRODUCT
WHERE PRO_STAT = 3
ORDER BY PRO_DATE;


-- 이 값이 7 이하면 신상품이다. NEW 딱지가 붙는다.
SELECT 
    ROUND((SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD')) FROM DUAL) - PRO_DATE)
FROM PRODUCT;
...
FROM PRODUCT
WHERE (
    ROUND((SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD')) FROM DUAL) - PRO_DATE) < 7
);
